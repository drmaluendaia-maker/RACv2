...
io.on('connection', (socket) => {
    ...
    const setupProtectedEvents = () => {
        const events = {
            ...
            'call_patient': ({ id, consultorio }) => { if (userRole !== 'medico') return; const currentlyAttending = patients.find(pt => pt.doctor === currentUser.fullName && pt.status === 'atendiendo'); if (currentlyAttending) { currentlyAttending.status = currentlyAttending.previousStatus || 'en_espera'; delete currentlyAttending.consultorio; } const p = patients.find(p => p.id === id); if (p) { p.previousStatus = p.status; p.status = 'atendiendo'; p.consultorio = consultorio; p.doctor = currentUser.fullName; currentlyCalled = { nombre: p.nombre, consultorio }; io.emit('update_call', currentlyCalled); setTimeout(() => { currentlyCalled = null; io.emit('update_call', null); }, 20000); } },
            ...
            'continue_care': (patientId) => { if (userRole !== 'medico') return; const historyIndex = attendedHistory.findIndex(p => p.id === patientId); if (historyIndex > -1) { const [patientToReactivate] = attendedHistory.splice(historyIndex, 1); patientToReactivate.status = 'pre_internacion'; delete patientToReactivate.consultorio; delete patientToReactivate.doctor; patients.push(patientToReactivate); sortPatients(); } }
        };
